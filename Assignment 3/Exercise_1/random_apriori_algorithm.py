...
parser.add_argument('-f', '--data_file') # the file data to be called
parser.add_argument('-s', '--support_threshold', default=0.5) # define support threshold for different test cases
parser.add_argument('-c', '--confidence', default=0.5)  # define confidence for different test cases
parser.add_argument('-p', '--fixed_probability', default=0.5) # define sample size by probability
...
def main():
    # the starting time of running the program
    programStartTime = time.time()
    # firstly get the sample data from dataset
    data = process_file(args.data_file)

    single_itemset_list = get_single_itemset(data)
    ...
    candidate_list, support_value_2 = apriori_algo(data, float(args.support_threshold))
